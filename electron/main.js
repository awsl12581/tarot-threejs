...
function createWindow() {
  ...
  mainWindow.loadURL(NODE_ENV === 'development'
    ? 'http://localhost:8991'
    : format({
      pathname: path.join(__dirname, '../dist/index.html'), // relative path to the HTML-file
      protocol: 'file:',
      slashes: true,
    }));
  // mainWindow.loadURL(
  ...
  // );
  // mainWindow.loadURL('http://localhost:8991');
  ...
  app.whenReady().then(() => {
    ...
    protocol.handle('file', (request) => {
      net.fetch('../dist/' + request.url.slice('file://'.length)).then(r =>{
        return r;
      }
    );
    });
